...
virtual void compute(const vd::Time& /*time*/)
{
    ...
    // Surface saine, création par la croissance, destruction par l'infection et la senescence naturelle
    AreaHealthy = fmax(
        AreaHealthy(-1) 
        +RateAreaExpansion() 
        -(RateDeseaseTransmission() * AreaInfectious(-1) * AreaHealthy(-1)/AreaExpansion(-1)) 
        -(RateAreaSenescence() * AreaHealthy(-1)/AreaActive(-1))
        -(InDeposition()* AreaHealthy(-1)/AreaExpansion(-1)),0);
